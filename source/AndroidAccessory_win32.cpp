...
bool AndroidAccessory::switchDevice(byte addr)
{
    ...
    if (protocol >= 1) {
        printf("device supports protocol 1 or higher\n");
    } else {
        printf("could not read device protocol version\n");
        return false;
    }
...
bool AndroidAccessory::findEndpoints(byte addr, EP_RECORD *inEp, EP_RECORD *outEp)
{
    ...
    if (err) {
        printf("Can't get config descriptor length\n");
        return false;
    }
    ...
    if (len > sizeof(descBuff)) {
        printf("config descriptor too large\n");
    ...
    while (p < (descBuff + len)){
        ...
        default:
            printf("unkown desc type ");
            printfln( descType, HEX);
            break;
        }
    ...
    if (!(inEp->epAddr && outEp->epAddr))
        printfln("can't find accessory endpoints");
...
bool AndroidAccessory::configureAndroid(void)
{
    ...
    printfln(inEp.epAddr, HEX);
    printfln(outEp.epAddr, HEX);
...
bool AndroidAccessory::isConnected(void)
{
    ...
    if (!connected &&
        ...
        if (isAccessoryDevice(devDesc)) {
            printf("found android acessory device\n");

            connected = configureAndroid();
        } else {
            printf("found possible device. swithcing to serial mode\n");
            switchDevice(1);
        }
